...
/**
 * Final class for the main plugin functionality.
 *
 * @since 1.0.0
 */
final class Cache_Enabler {
	...
	public function __construct() {
		...
		add_action( 'init', array( __CLASS__, 'process_clear_micro_cache_endpoint' ) );
		...
		add_action( 'cache_enabler_clear_micro_cache', array( __CLASS__, 'clear_micro_cache' ) );
	...
	public static function on_upgrade( $upgrader, $data ) {
		...
		if ( 'theme' === $data['type'] && isset( $data['themes'] ) ) {
		...
		if ( 'plugin' === $data['type'] && isset( $data['plugins'] ) ) {
	...
	public static function update_backend() {
		...
		if ( false === has_action( 'update_option', array( __CLASS__, 'on_update_option' ) ) ) {
	...
	private static function uninstall_backend() {
		...
		delete_transient( 'cache_enabler_cache_size' );
		delete_transient( 'cache_enabler_micro_cache_size' );
	...
	public static function get_settings( $update = true ) {
		...
		if ( false === $settings || ! isset( $settings['version'] ) || CACHE_ENABLER_VERSION !== $settings['version'] ) {
			if ( $update ) {
				self::update();
	 ...
	 *
	 * @since  1.5.0
	 * @param  string $type Type of cache cleared ('site' or 'micro').
	 * @return  string  Name of the transient.
	 */
	private static function get_cache_cleared_transient_name( $type = 'site' ) {

		$transient_name = 'cache_enabler_cache_cleared_' . $type . '_' . get_current_user_id();
	...
	private static function get_default_user_settings() {
		...
		$default_user_settings = array(
			...
			'micro_cache_duration'               => 900, // 15 minutes in seconds.
			'micro_cache_urls'                   => '',
			'micro_cache_secret_key'             => '',
		);
	...
	private static function upgrade_settings( $settings ) {
		...
		// < 1.5.0.
		if ( isset( $settings['minify_html'] ) && 2 === $settings['minify_html'] ) {
	...
	public static function add_plugin_row_meta( $plugin_meta, $plugin_file ) {
		...
		if ( CACHE_ENABLER_BASE !== $plugin_file ) {
			return $plugin_meta;
		}
	...
	public static function add_admin_bar_items( $wp_admin_bar ) {
		...
		// Add main parent menu item.
		$wp_admin_bar->add_menu(
			array(
				'id'     => 'cache_enabler_clear_cache_menu',
		...
		// Add "Clear Site Cache" submenu item.
		$site_cache_title = ( is_multisite() && is_network_admin() ) ? esc_html__( 'Clear Network Cache', 'cache-enabler' ) : esc_html__( 'Clear Site Cache', 'cache-enabler' );
		$wp_admin_bar->add_menu(
			array(
				'id'     => 'cache_enabler_clear_site_cache',
				'parent' => 'cache_enabler_clear_cache_menu',
				'href'   => wp_nonce_url(
					add_query_arg(
						array(
				...
				'title'  => '<span class="ab-item">' . $site_cache_title . '</span>',
				'meta'   => array( 'title' => $site_cache_title ),
			)
		);

		// Add "Clear Micro Cache" submenu item.
		$wp_admin_bar->add_menu(
			array(
				'id'     => 'cache_enabler_clear_micro_cache',
				'parent' => 'cache_enabler_clear_cache_menu',
				'href'   => wp_nonce_url(
					add_query_arg(
						array(
							'_cache'  => 'cache-enabler',
							'_action' => 'clearmicrocache',
						)
					),
					'cache_enabler_clear_cache_nonce'
				),
				'title'  => '<span class="ab-item">' . esc_html__( 'Clear Micro Cache', 'cache-enabler' ) . '</span>',
				'meta'   => array( 'title' => esc_html__( 'Clear Micro Cache', 'cache-enabler' ) ),
			)
		);

		// Add "Clear Page Cache" submenu item (if not in admin).
		if ( ! is_admin() ) {
			$wp_admin_bar->add_menu(
				array(
	...
	public static function add_admin_resources( $hook ) {
		...
		if ( 'settings_page_cache-enabler' === $hook ) {
	...
	public static function process_clear_cache_request() {
		...
		$get_cache  = isset( $_GET['_cache'] ) ? sanitize_key( $_GET['_cache'] ) : '';
		$get_action = isset( $_GET['_action'] ) ? sanitize_key( $_GET['_action'] ) : '';
		$get_nonce  = isset( $_GET['_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) : '';

		if ( empty( $get_cache ) || empty( $get_action ) || 'cache-enabler' !== $get_cache || ! in_array( $get_action, array( 'clear', 'clearurl', 'clearmicrocache' ), true ) ) {
			return;
		}

		if ( empty( $get_nonce ) || ! wp_verify_nonce( $get_nonce, 'cache_enabler_clear_cache_nonce' ) ) {
			return;
		}
		...
		if ( 'clearurl' === $get_action ) {
			$request_uri = isset( $_SERVER['REQUEST_URI'] ) ? esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';
			self::clear_page_cache_by_url( Cache_Enabler_Engine::$request_headers['Host'] . Cache_Enabler_Engine::sanitize_server_input( $request_uri, false ) );
		} elseif ( 'clear' === $get_action ) {
			self::each_site( ( is_multisite() && is_network_admin() ), self::class . '::clear_site_cache', array(), true );
		} elseif ( 'clearmicrocache' === $get_action ) {
			self::clear_micro_cache();
			$transient_type = 'micro';
		}

		if ( is_admin() ) {
			set_transient( self::get_cache_cleared_transient_name( $transient_type ), 1 );
		}
	...
	/**
	 * Process clear micro cache endpoint request.
	 *
	 * This runs on the 'init' action and clears the micro cache if the secret key matches.
	 *
	 * @since 2.0.0
	 */
	public static function process_clear_micro_cache_endpoint() {

		if ( isset( $_GET['ce_clear_micro_cache'] ) ) {
			// Nonce verification for security.
			$nonce = isset( $_GET['_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) : '';
			if ( empty( $nonce ) || ! wp_verify_nonce( $nonce, 'cache_enabler_clear_micro_cache' ) ) {
				wp_send_json_error( array( 'message' => 'Invalid or missing nonce.' ), 403 );
				exit;
			}

			// Get settings directly from the database to avoid race conditions with engine initialization.
			$settings     = get_option( 'cache_enabler', array() );
			$secret_key   = $settings['micro_cache_secret_key'] ?? '';
			$provided_key = sanitize_text_field( wp_unslash( $_GET['ce_clear_micro_cache'] ) );

			if ( ! empty( $secret_key ) && hash_equals( $secret_key, $provided_key ) ) {
				// Key is valid. Ensure the engine is started before clearing cache,
				// as clear_micro_cache() depends on its settings property.
				if ( ! class_exists( 'Cache_Enabler_Engine' ) || ! Cache_Enabler_Engine::$started ) {
					Cache_Enabler_Engine::start();
				}

				self::clear_micro_cache();
				wp_send_json_success( array( 'message' => 'Micro cache cleared.' ) );
			} else {
				wp_send_json_error( array( 'message' => 'Invalid secret key.' ), 403 );
			}
			exit;
		}
	}
	...
	public static function cache_cleared_notice() {
		...
		if ( get_transient( self::get_cache_cleared_transient_name( 'site' ) ) ) {
			printf(
			...
			delete_transient( self::get_cache_cleared_transient_name( 'site' ) );
		}

		if ( get_transient( self::get_cache_cleared_transient_name( 'micro' ) ) ) {
			printf(
				'<div class="notice notice-success is-dismissible"><p><strong>%s</strong></p></div>',
				esc_html__( 'Micro cache cleared.', 'cache-enabler' )
			);
			delete_transient( self::get_cache_cleared_transient_name( 'micro' ) );
		}
	}
	...
	/**
	 * Clear the micro cache for the current site.
	 *
	 * @since 2.0.0
	 */
	public static function clear_micro_cache() {
		$urls = Cache_Enabler_Engine::$settings['micro_cache_urls'] ?? '';

		if ( ! empty( $urls ) ) {
			$url_list = preg_split( '/\r\n|\r|\n/', $urls );
			foreach ( $url_list as $relative_url ) {
				$relative_url = trim( $relative_url );
				if ( ! empty( $relative_url ) ) {
					$url_to_clear = home_url( $relative_url );
					self::clear_page_cache_by_url( $url_to_clear );
				}
			}
		}

		do_action( 'cache_enabler_micro_cache_cleared' );
	}
	...
	public static function clear_post_type_archive_cache( $post = null ) {
		...
		if ( $post instanceof WP_Post ) {
			...
			if ( false !== $post_type_archive_url && false === strpos( $post_type_archive_url, '?' ) ) {
	...
	public static function clear_post_date_archives_cache( $post = null ) {
		...
		if ( $post instanceof WP_Post ) {
			...
			foreach ( $date_archive_urls as $date_archive_url ) {
				if ( false === strpos( $date_archive_url, '?' ) ) {
	...
	public static function clear_term_archive_cache( $term, $taxonomy = '' ) {
		...
		if ( $term instanceof WP_Term ) {
			...
			if ( ! is_wp_error( $term_archive_url ) && false === strpos( $term_archive_url, '?' ) ) {
	...
	public static function clear_author_archive_cache( $author = null ) {
		...
		if ( $author instanceof WP_User ) {
			...
			if ( false === strpos( $author_archive_url, '?' ) ) {
	...
	public static function clear_page_cache_by_post( $post, $args = array() ) {
		...
		if ( $post instanceof WP_Post ) {
			...
			if ( false !== $post_url && false === strpos( $post_url, '?' ) ) {
		...
		if ( version_compare( PHP_VERSION, CACHE_ENABLER_MIN_PHP, '<' ) ) {
			printf(
				...
				sprintf(
					...
					esc_html( CACHE_ENABLER_MIN_PHP )
				)
			);
		}
		...
		if ( version_compare( $GLOBALS['wp_version'], CACHE_ENABLER_MIN_WP . 'alpha', '<' ) ) {
			printf(
				...
				sprintf(
					...
					'<strong>Cache Enabler</strong>',
					esc_html( CACHE_ENABLER_MIN_WP )
				)
			);
		}

		// Check the advanced-cache.php drop-in file.
		if ( ! file_exists( WP_CONTENT_DIR . '/advanced-cache.php' ) && false === Cache_Enabler_Disk::create_advanced_cache_file() ) {
			printf(
				...
				sprintf(
					...
					'<code>' . esc_html( CACHE_ENABLER_DIR ) . '</code>',
		...
		if ( ! file_exists( CACHE_ENABLER_INDEX_FILE ) ) {
			printf(
				...
				sprintf(
					...
					'<strong>Cache Enabler</strong>',
					'<code>' . esc_html( CACHE_ENABLER_INDEX_FILE ) . '</code>',
		...
		if ( defined( 'AUTOPTIMIZE_PLUGIN_DIR' ) && Cache_Enabler_Engine::$settings['minify_html'] && '' !== get_option( 'autoptimize_html', '' ) ) {
			printf(
				...
				sprintf(
					...
					sprintf(
						'<a href="%s">%s</a>',
						esc_url( admin_url( 'options-general.php?page=cache-enabler' ) ),
	...
	public static function schedule_events() {
		...
		foreach ( $events as $hook => $recurrence ) {
			if ( 'cache_enabler_clear_expired_cache' === $hook ) {
				if ( ! Cache_Enabler_Engine::$settings['cache_expires'] || 0 === Cache_Enabler_Engine::$settings['cache_expiry_time'] ) {
					continue;
				}
			}
	...
	public static function validate_settings( $settings ) {
		$old_settings = get_option( 'cache_enabler', self::get_default_settings() );

		$submitted_tab = '';
		if ( isset( $_POST['cache_enabler_submitted_tab'] ) ) {
			// Verify nonce before processing form data.
			if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'cache_enabler-options' ) ) {
				return $old_settings; // Abort if nonce is missing or invalid.
			}
			$submitted_tab = sanitize_text_field( wp_unslash( $_POST['cache_enabler_submitted_tab'] ) );
		}

		// Validation for Micro Caching settings.
		if ( 'micro_caching' === $submitted_tab ) {
			$excluded_page_paths_regex = $old_settings['excluded_page_paths'];
			$micro_cache_urls          = isset( $settings['micro_cache_urls'] ) ? preg_split( '/\r\n|\r|\n/', $settings['micro_cache_urls'] ) : array();

			if ( ! empty( $excluded_page_paths_regex ) ) {
				foreach ( $micro_cache_urls as $url ) {
					$url = trim( $url );
					if ( ! empty( $url ) && 1 === preg_match( $excluded_page_paths_regex, $url ) ) {
						add_settings_error(
							'cache_enabler',
							'micro_cache_conflict',
							// translators: %s: The conflicting URL.
							sprintf( __( "A page can't be both excluded from caching and be micro-cached. The URL '%s' matches the cache exclusion pattern. Settings not saved.", 'cache-enabler' ), esc_html( $url ) ),
							'error'
						);
						return $old_settings; // Return old settings to prevent saving.
					}
				}
			}
		}

		// Validation for Site Caching settings.
		if ( 'site_caching' === $submitted_tab ) {
			$excluded_page_paths_regex = isset( $settings['excluded_page_paths'] ) ? self::validate_regex( $settings['excluded_page_paths'] ) : '';
			$micro_cache_urls          = isset( $old_settings['micro_cache_urls'] ) ? preg_split( '/\r\n|\r|\n/', $old_settings['micro_cache_urls'] ) : array();
			...
			if ( ! empty( $excluded_page_paths_regex ) ) {
				foreach ( $micro_cache_urls as $url ) {
					...
					if ( ! empty( $url ) && 1 === preg_match( $excluded_page_paths_regex, $url ) ) {
						add_settings_error(
							'cache_enabler',
							'cache_exclusion_conflict',
							// translators: %s: The conflicting URL.
							sprintf( __( "The updated 'Page Paths' exclusion pattern matches a URL '%s' in the Micro Caching list. Settings not saved.", 'cache-enabler' ), esc_html( $url ) ),
							'error'
						);
						return $old_settings; // Return old settings to prevent saving.
					}
		...
		// Merge new settings with old ones to prevent data loss from the other tab.
		$settings = wp_parse_args( $settings, $old_settings );
		...
		$validated_settings = wp_parse_args(
			array(
				...
				'micro_cache_duration'               => absint( $settings['micro_cache_duration'] ),
				'micro_cache_urls'                   => (string) sanitize_textarea_field( $settings['micro_cache_urls'] ),
				'micro_cache_secret_key'             => (string) sanitize_text_field( $settings['micro_cache_secret_key'] ),
			),
			self::get_default_settings( 'system' )
		);

		if ( ! empty( $_POST['cache_enabler']['clear_site_cache_on_saved_settings'] ) ) {
			self::clear_site_cache();
			set_transient( self::get_cache_cleared_transient_name( 'site' ), 1 );
		}

		if ( ! empty( $_POST['cache_enabler']['clear_micro_cache_on_saved_settings'] ) ) {
			self::clear_micro_cache();
			set_transient( self::get_cache_cleared_transient_name( 'micro' ), 1 );
		}
	...
	public static function settings_page() {
		...
		<div id="cache_enabler_settings" class="wrap">
			...
			<?php
			// The Settings API automatically displays saved/error messages.
			...
			<h2 class="nav-tab-wrapper">
				<a href="#site-caching" class="nav-tab nav-tab-active"><?php esc_html_e( 'Site Caching', 'cache-enabler' ); ?></a>
				<a href="#micro-caching" class="nav-tab"><?php esc_html_e( 'Micro Caching', 'cache-enabler' ); ?></a>
			</h2>
			...
			<form method="post" action="options.php">
				...
				<div id="site-caching" class="tab-content active-tab">
					<input type="hidden" name="cache_enabler_submitted_tab" value="site_caching" />
								...
								<fieldset>
									...
									<label for="cache_enabler_cache_expiry_time">
										...
										printf(
											...
											'<input name="cache_enabler[cache_expiry_time]" type="number" id="cache_enabler_cache_expiry_time" value="' . esc_attr( (string) Cache_Enabler_Engine::$settings['cache_expiry_time'] ) . '" class="small-text">'
										);
										?>
									</label>
									...
									<label for="cache_enabler_minify_inline_css_js">
										...
										printf(
											...
											wp_kses_post( $minify_inline_css_js )
										);
										?>
									</label>
								</fieldset>
					...
					<p class="submit">
						<input type="submit" class="button-secondary" value="<?php esc_attr_e( 'Save Changes', 'cache-enabler' ); ?>" />
						<input name="cache_enabler[clear_site_cache_on_saved_settings]" type="submit" class="button-primary" value="<?php esc_attr_e( 'Save and Clear Site Cache', 'cache-enabler' ); ?>" />
					</p>
				</div>


				<div id="micro-caching" class="tab-content">
					<input type="hidden" name="cache_enabler_submitted_tab" value="micro_caching" disabled/>
					<table class="form-table">
						<tr valign="top">
							<th scope="row">
								<?php esc_html_e( 'Micro Cache Settings', 'cache-enabler' ); ?>
							</th>
							<td>
								<fieldset>
									<p class="subheading"><?php esc_html_e( 'Duration', 'cache-enabler' ); ?></p>
									<label for="cache_enabler_micro_cache_duration">
										<select name="cache_enabler[micro_cache_duration]" id="cache_enabler_micro_cache_duration">
											<?php
											$durations = array(
												'15 Minutes' => 900,
												'30 Minutes' => 1800,
											...
											foreach ( $durations as $label => $seconds ) {
												echo '<option value="' . esc_attr( $seconds ) . '"' . selected( $seconds, Cache_Enabler_Engine::$settings['micro_cache_duration'], false ) . '>' . esc_html( $label ) . '</option>';
											}
											?>
										</select>
										<p class="description"><?php esc_html_e( 'Set the duration for how long micro-cached pages should be stored.', 'cache-enabler' ); ?></p>
									</label>

									<br />

									<p class="subheading"><?php esc_html_e( 'URLs to Micro Cache', 'cache-enabler' ); ?></p>
									<label for="cache_enabler_micro_cache_urls">
										<textarea name="cache_enabler[micro_cache_urls]" rows="10" cols="50" id="cache_enabler_micro_cache_urls" class="large-text code"><?php echo esc_textarea( Cache_Enabler_Engine::$settings['micro_cache_urls'] ); ?></textarea>
										<p class="description"><?php esc_html_e( 'Enter the relative URLs to micro-cache, one per line. These URLs cannot match any pattern in "Cache Exclusions" > "Page Paths".', 'cache-enabler' ); ?></p>
										<p><?php esc_html_e( 'Example:', 'cache-enabler' ); ?> <code class="code--form-control">/shop/</code><br /><?php esc_html_e( 'Example:', 'cache-enabler' ); ?> <code class="code--form-control">/product-category/new-arrivals/</code></p>
						...
						<tr valign="top">
							<th scope="row">
								<?php esc_html_e( 'Clearing Endpoint', 'cache-enabler' ); ?>
							</th>
							<td>
								<fieldset>
									<p class="subheading"><?php esc_html_e( 'Secret Key', 'cache-enabler' ); ?></p>
									<label for="cache_enabler_micro_cache_secret_key">
										<input name="cache_enabler[micro_cache_secret_key]" type="text" id="cache_enabler_micro_cache_secret_key" value="<?php echo esc_attr( Cache_Enabler_Engine::$settings['micro_cache_secret_key'] ); ?>" class="regular-text" />
										<p class="description">
											<?php esc_html_e( 'A secret key to include in the endpoint URL for clearing the micro-cache.', 'cache-enabler' ); ?>
											<br>
											<?php
											$secret_key   = Cache_Enabler_Engine::$settings['micro_cache_secret_key'];
											$endpoint_key = ! empty( $secret_key ) ? $secret_key : 'YOUR_SECRET_KEY';
											$endpoint_url = home_url( '/?ce_clear_micro_cache=' . $endpoint_key );
											esc_html_e( 'Use the following URL with cURL or wget to clear the cache:', 'cache-enabler' );
											?>
											<br>
					...
					<p class="submit">
						<input type="submit" class="button-secondary" value="<?php esc_attr_e( 'Save Changes', 'cache-enabler' ); ?>" />
						<input name="cache_enabler[clear_micro_cache_on_saved_settings]" type="submit" class="button-primary" value="<?php esc_attr_e( 'Save and Clear Micro Cache', 'cache-enabler' ); ?>" />
					</p>
				</div>
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			const tabLinks = document.querySelectorAll('.nav-tab-wrapper a.nav-tab');
			const tabContents = document.querySelectorAll('.tab-content');
			const submittedTabInputs = document.querySelectorAll('.tab-content input[name="cache_enabler_submitted_tab"]');
			const refererInput = document.querySelector('form input[name="_wp_http_referer"]');

			// Tab-switching logic on click
			tabLinks.forEach(function(link) {
				link.addEventListener('click', function(e) {
					e.preventDefault();
					
					const tab_id = this.getAttribute('href').substring(1); // e.g., 'micro-caching'
					...
					// Update classes for content panes
					tabContents.forEach(function(content) {
						content.classList.remove('active-tab');
					});
					document.querySelector(targetSelector).classList.add('active-tab');
					
					// Update the hidden input to identify which form is being submitted
					submittedTabInputs.forEach(function(input) {
						input.disabled = true;
					});
					document.querySelector(targetSelector).querySelector('input[name="cache_enabler_submitted_tab"]').disabled = false;

					// Update the _wp_http_referer to include the new tab for the redirect
					if (refererInput) {
						let referer_url = refererInput.value;
						// Remove any old 'tab' parameter
						referer_url = referer_url.replace(/&tab=[^&]*/, '');
						// Append the new 'tab' parameter
						refererInput.value = referer_url + '&tab=' + tab_id;
					}
				});
			});

			// On page load, check for the 'tab' query parameter and switch to it.
			const active_tab = getParameterByName('tab');
			if (active_tab) {
				const tabToActivate = document.querySelector('a.nav-tab[href="#' + active_tab + '"]');
				if (tabToActivate) {
					// We just need to trigger the click handler to run all the logic
					tabToActivate.click();
				}
			}
		});
